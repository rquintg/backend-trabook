...
export const createTravel = async (req, res) => {
    const {pais, city, price,img, price_off, score} = req.body
    try {

        const [rows] = await pool.query('INSERT INTO travels (pais, city, price, img, price_off, score) VALUES (?, ?, ?, ?, ?, ?)', [pais, city, price, img, price_off, score])
        res.send({
...
export const updateTravel = async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('UPDATE travels SET pais = IFNULL(?, pais), city = IFNULL(?, city), price = IFNULL(?,price), img = ifnull(?, img), price_off = ifnull(?, price_off), score = IFNULL(?, score) WHERE idtravels = ?', [pais, city, price, img, price_off, score, id])
